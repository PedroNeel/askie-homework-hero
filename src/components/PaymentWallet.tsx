  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold gradient-text mb-2">Your Askie Wallet</h1>
      ...
      <Card className="p-6 bg-gradient-sunset text-white">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sunset-100 mb-1">Available Balance</p>
            <p className="text-3xl font-bold">R{balance.toFixed(2)}</p>
            <p className="text-sunset-200 text-sm">≈ {Math.floor(balance / 2.5)} questions</p>
          </div>
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {paymentProviders.map((provider) => (
                <Card
                  key={provider.id}
                  className={`p-4 cursor-pointer hover-lift border-2 ${
                    selectedProvider === provider.id ? 'border-sunset-300 bg-sunset-50' : 'border-gray-200'
                  }`}
